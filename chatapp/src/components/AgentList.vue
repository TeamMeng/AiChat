<template>
    ...
    >
        <div class="bg-gradient-to-br from-[#1e1e2e] to-[#181825] rounded-2xl w-[500px] max-h-[85vh] border border-[#313244] shadow-2xl flex flex-col">
            <!-- Header (fixed) -->
            ...
            <div class="flex-1 overflow-y-auto px-6 pb-6 space-y-4">
                ...
                <div v-if="!showCreateAgentForm">
                    ...
                    >
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div class="text-[#9399b2] text-xs">{{ agent.type }} · {{ agent.adapter }} · {{ agent.model }}</div>
                            </div>
                        </div>
                        <button
                    ...
                    <button
                        ...
                        class="w-full py-3 mt-2 bg-gradient-to-r from-[#cba6f7] to-[#94e2d5] hover:from-[#94e2d5] hover:to-[#89dceb] text-[#1e1e2e] rounded-lg font-medium transition-all duration-200 flex items-center justify-center gap-2"
                    >
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Create Agent Form -->
                <div v-else>
                    <h3 class="text-[#cdd6f4] font-semibold mb-4">Create New Agent</h3>
                    <form @submit.prevent="handleCreateAgent" class="space-y-4">
                        ...
                        <div v-if="errorMessage" class="flex items-start gap-2 p-3 bg-[#45475a] border border-[#f38ba8]/30 rounded-lg">
                            <svg class="w-4 h-4 text-[#f38ba8] mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="flex gap-2 pt-1">
                            ...
                            <button
                                type="submit"
                                :disabled="isCreating"
                                class="flex-1 py-2 bg-gradient-to-r from-[#cba6f7] to-[#94e2d5] hover:from-[#94e2d5] hover:to-[#89dceb] text-[#1e1e2e] rounded-lg font-medium transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
...
export default {
    ...
    methods: {
        ...
        cancelCreate() {
            ...
            this.errorMessage = "";
            this.newAgent = { name: "", model: "", prompt: "", type: "proxy", adapter: "ollama" };
        },
