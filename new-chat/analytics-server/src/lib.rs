...
use chat_core::{
    DecodingKey, User,
    middlewares::{TokenVerify, extract_user},
};
...
use crate::{handlers::create_event_handler, openapi::OpenApiRouter};
...
impl AppState {
    pub async fn try_new(config: AppConfig) -> Result<Self, AppError> {
        ...
        if let (Some(user), Some(password)) = (
            config.server.db_user.as_ref(),
            config.server.db_password.as_ref(),
        ) {
