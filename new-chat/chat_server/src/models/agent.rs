...
impl AppState {
    ...
    pub async fn update_agent(
        ...
        if !self.agent_id_exists(chat_id, input.id).await? {
            info!(
                "Agent by id {} already exists in chat {}",
                input.id, chat_id
            );
            return Err(AppError::UpdateAgentError(format!(
                "Agent by id: {} not exists",
                input.id
            )));
        }
        ...
        let agent = match (prompt.as_str(), &args) {
            ("", _) => {
                sqlx::query_as(
                    "
                    UPDATE chat_agents
                    SET args = $1 WHERE chat_id = $2 AND id = $3
                    RETURNING *
                    ",
                )
                .bind(args)
            ...
            (_, _) => {
                sqlx::query_as(
                    "
                    UPDATE chat_agents
                    SET prompt = $1, args = $2 WHERE chat_id = $3 AND id = $4
                    RETURNING *
                    ",
                )
                .bind(prompt)
